...
the engineering team has determined that it can only handle a model that has no more than 600 parameters.
On the other hand,
the product management team has determined that the model is not launchable unless the LogLoss is less than 0.35 on the holdout test set.
use L1 regularization—to tune the model to satisfy both the size and accuracy constraints?
'''
'''
...
linear_classifier = train_linear_classifier_model(
    ...
    # A regularization strength of 0.1 should be sufficient.
    # Note that there is a compromise to be struck: stronger regularization gives us smaller models, but can affect the classification loss.
